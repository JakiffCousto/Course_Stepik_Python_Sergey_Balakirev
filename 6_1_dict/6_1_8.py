...
'''
import sys
lst_test = ['ustanovka-i-zapusk-yazyka',
          'ustanovka-i-poryadok-raboty-pycharm',
          'peremennyye-operator-prisvaivaniya-tipy-dannykh',
          'arifmeticheskiye-operatsii',
          'ustanovka-i-poryadok-raboty-pycharm']
...
for i in range(len(lst_test)):
    if lst_test[i] not in d:
        d[lst_test[i]] = lst_test[i]
        print("HTML-страница для адреса ", lst_test[i])
    else:
        print("Взято из кэша: HTML-страница для адреса ", d[lst_test[i]])
